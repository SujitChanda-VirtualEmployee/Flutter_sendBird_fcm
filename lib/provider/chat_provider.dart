...
import 'package:flutter_sendbird_fcm/screens/chat_screen/components/attachment_modal.dart';
...
import 'package:flutter_sendbird_fcm/utility/debounce.dart';
...
class ChatProvider
  ...
  void onSendFileMessage(File file, String peerName, String peerToken, String channelUrl1) async {
  ...
  void showPlusMenu(
      BuildContext context, String peerName, String peerToken, channelUrl1) async {
    final modal = AttachmentModal(context: context);
    final file = await modal.getFile();
    if (file != null) onSendFileMessage(file, peerName, peerToken, channelUrl1);
  }
  ...
  PopupMenuEntry _buildPopupItem(
      String text, IconData imageName, PopupMenuType value) {
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              Text(text),
              SizedBox(width: 8),
              Icon(imageName, color: Colors.blue)
            ],
          ),
        ),
        value: value);
